...
| Counted as recorded | `covered` | `src/protocol/voting-ballot-close.ts`, `src/protocol/voting-verification.ts`, `tests/node/integration/public-voting-flow.test.ts` | The verifier proves which complete ballots were counted after `ballot-close` and recomputes the exact tallies from that counted subset. |
