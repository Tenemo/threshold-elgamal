---
title: "fixedHexToBigint"
editUrl: false
---
[**threshold-elgamal**](../../)

***

[threshold-elgamal](../../modules/) / [threshold-elgamal](../) / fixedHexToBigint

# Function: fixedHexToBigint()

> **fixedHexToBigint**(`hex`): `bigint`

Decodes a fixed-width hexadecimal string back into a bigint.

## Parameters

### hex

`string`

Lowercase or uppercase hexadecimal input.

## Returns

`bigint`

The decoded bigint value.

## Throws

[InvalidPayloadError](../../core/classes/InvalidPayloadError/) When the input is not valid hexadecimal.
